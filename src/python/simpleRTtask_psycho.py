...
n_trialsBlock = 30
n_trialsTotal = 90 + n_trialsBlock # to account for first training block
n_blocks = int(n_trialsTotal/n_trialsBlock)
bslBlocks = 3 # baseline every how many blocks
iti = 0
bslDur = 30 # duration in s
n_corr_train = 1 # 10 # number of consecutive correct trials required for the first training block
...
def save_events(event_log):
    filepath = f'..\\..\\rec\\simpleRT\\pilot\\sub-P{"%03d" % subject}\\ses-S001\\eeg'
